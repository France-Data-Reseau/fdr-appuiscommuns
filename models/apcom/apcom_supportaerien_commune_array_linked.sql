...
with apcomsup as (
    ...
    {# OLD %- set fields = ['_dbt_source_relation', 'apcomsup_src_priority'] + adapter.get_columns_in_relation(ref('appuiscommuns_supportaerien__definition')) | map(attribute="name") | list -%#}-- BEWARE without | list it stays a generator that can only be iterated once
