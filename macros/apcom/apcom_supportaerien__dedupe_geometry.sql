...
with link_candidates as (
    ...
    select
        {# % for field in id_and_order_by_fields %}
          earlier."{{ field }}" as "earlier{{ field }}", later."{{ field }}" as "later{{ field }}" {% if not loop.last%},{% endif %}
        {% endfor % #}
    ...
    ORDER BY earlier."{{ src_name_field }}" asc, earlier."{{ src_id_field }}" desc,
  ...
  where "earlier{{ id_field }}" not in (
    select "later{{ id_field }}" from link_candidates lc_before_current_one
    WHERE lc_before_current_one."earlier{{ src_name_field }}" < current_lc."later{{ src_name_field }}" -- only between different sources
    --and earlier."{{ src_id_field }}" < later."{{ src_id_field }}" -- ONLY IF WERE IN SAME SOURCE assuming _src_id is the model's ordering field
  )
  ...
  order by "earlier{{ src_name_field }}" asc, "earlier{{ src_id_field }}" desc, "later{{ src_name_field }}", "later{{ src_id_field }}"
